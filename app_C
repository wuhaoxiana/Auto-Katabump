...
# 从环境变量获取账号密码和 TG 配置
EMAIL_C        = os.environ.get("KATABUMP_EMAIL_C") or ""    # 登录邮箱
PASSWORD_C     = os.environ.get("KATABUMP_PASSWORD_C") or "" # 账号密码
...
def send_tg_message(status_icon, status_text, time_left=""):
    ...
    if '@' in EMAIL_C:
        name, domain = EMAIL_C.split('@', 1)
        if len(name) > 4:
            masked_email = f"{name[:2]}****{name[-2:]}@{domain}"
        else:
            masked_email = f"{name}@{domain}"
    else:
        masked_email = EMAIL_C[:2] + '****'
...
def login(sb) -> bool:
    ...
    print(f"📧 填写邮箱...")
    js_fill_input(sb, 'input[name="email"]', EMAIL_C)
    time.sleep(0.3)
    
    print("🔑 填写密码...")
    js_fill_input(sb, 'input[name="password"]', PASSWORD_C)
    time.sleep(1)
...
def _solve_altcha(sb) -> bool:
    ...
    # 先检查是否已自动通过
    if sb.execute_script(_CLTCHA_SOLVED_JS):
        print("✅ ALTCHA 已自动通过")
        return True
    ...
    coords = None
    try:
        coords = sb.execute_script(_CLTCHA_EXPAND_JS)
    except Exception:
        pass
    ...
    for attempt in range(3):
        ...
        try:
            new_coords = sb.execute_script(_CLTCHA_EXPAND_JS)
            if new_coords:
                coords = new_coords
        except Exception:
            pass
